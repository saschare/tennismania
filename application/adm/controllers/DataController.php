...
class DataController extends Zend_Controller_Action {
    ...
    protected function _loadCategoryPlugins($idcat) {
        ...
        foreach ($namespaces as $namespace) {
            ...
            foreach ($plugins as $plugin) {
                ...
                if ($pluginPathInfo[1] == 'Category') {

                    if (Aitsu_Adm_User::getInstance()->isAllowed(array('area' => 'plugin.' . strtolower($pluginPathInfo[0]) . '.category'))) {
                        include_once ($plugin);

                        $registry = call_user_func(array(
                            $namespace . '_Plugin_' . $pluginPathInfo[0] . '_Category_Controller',
                            'register'
                                ), $idcat);
